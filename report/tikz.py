...
class Path(object):
    ...
    def edge_to(self, to, label=None, style="fill=white,anchor=center,pos=0.5,font=\\fontsize{10}{0}\\selectfont"):
        c = "edge[]"
        self.add_command(c, to=False)
        if label:
            c = "node[text,%s] {%s}" % (style, label)
            self.add_command(c)
        c = "(%s)" % (to,)
        self.add_command(c)
        return self
...
class Picture(object):
    ...
    def set_coord(self, pos, options='', name='', coord='abs'):
        cmd = '\\coordinate[%s] (%s) at %s;' % (
            options, name, format_pos(pos, coord))
        self.add_command(cmd)

    def set_node(self, pos, text='', options='', name='', coord='abs'):
        cmd = '\\node[%s] (%s) at %s {%s};' % (
            options, name, format_pos(pos, coord), text)
        self.add_command(cmd)
...
def main():
    ...
    with pic.path('draw') as draw:
        draw.at(3 + 0j, name='start').grid_to(6 + 6j)
