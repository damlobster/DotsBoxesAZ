...
class UCTNode():
  ...
  def __init__(self, game_state: GameState, move: int, parent=None):
    ...
    self.parent = parent 
    self.children = {}
    self.child_priors = np.zeros([game_state.get_actions_size()], dtype=np.float32)
    self.child_total_value = np.zeros([game_state.get_actions_size()], dtype=np.float32)
    self.child_number_visits = np.zeros([game_state.get_actions_size()], dtype=np.float32)
...
def UCT_search(root_node: UCTNode, num_reads, nn, cpuct=1.0):
  ...
  for _ in range(num_reads):
    ...
    if not leaf.is_terminal:
      ...
      child_priors, value_estimate = np.zeros(leaf.game_state.get_actions_size()), leaf.game_state.get_result()
